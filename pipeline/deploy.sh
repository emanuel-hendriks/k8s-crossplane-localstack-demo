...
echo "============="
echo ""
echo -e "${BLUE}Task 1 Namespaces:${NC}"
kubectl get namespaces | grep -E "(NAME|crossplane-system|default|localstack)"
echo ""
echo -e "${BLUE}Task 1 Pods:${NC}"
kubectl get pods -o wide | grep -E "(NAME|producer|consumer|dynamodb-admin)"
kubectl get pods -n localstack -o wide | grep -E "(NAME|localstack)"
kubectl get pods -n crossplane-system -o wide | grep -E "(NAME|crossplane|provider-aws)"
echo ""
echo -e "${BLUE}Task 1 Services:${NC}"
kubectl get services | grep -E "(NAME|producer|consumer|dynamodb-admin)"
kubectl get services -n localstack | grep -E "(NAME|localstack)"
echo ""
echo -e "${BLUE}Task 1 Docker Containers:${NC}"
docker ps --format "table {{.ID}}\t{{.Image}}\t{{.Status}}\t{{.Names}}" | grep -E "(CONTAINER|producer|consumer|dynamodb-admin|localstack|crossplane)"
